    ...
    size_t error_capacity)
{
    ...
    if (request_body == NULL || response == NULL || http_status == NULL ||
        error == NULL || error_capacity == 0U) {
        return false;
    }
    ...
    *http_status = 0L;
    ...
    (void)curl_easy_setopt(curl, CURLOPT_URL, PNX_XBOX_USER_AUTH_URL);
    ...
    PnxXboxStatus *status)
{
    ...
    status->stage = PNX_XBOX_CONNECTING;
